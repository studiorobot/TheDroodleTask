...
class statelessConversation:
    ...
    def __init__(self, model: str, prompts: list[str], conversationName: str, savePath: str = 'conversationArchive', 
                 tokenTracker: tokenTracker = None, timeFlattening: int = 0, history: bool = True):
        ...
        self._history = history  # New parameter to toggle history

        # Initialize conversation storage only if history is enabled
        self._conversation = [] if history else None
        self._conversationInternal = [] if history else None
    ...
    def contConversationDict(self, newMessage: dict) -> dict:
        ...
        # If history is enabled, store the message
        if self._history:
            self.insertMessageDict(newMessage)

        outMessage = self.turnoverConversationDict(newMessage)
        self._flattenTime(startTime)  # Flatten time if needed
        return outMessage
    ...
    def turnoverConversationDict(self, newMessage: dict) -> dict:
        # Prepare the conversation for the API request
        if self._history:
            conversation = self._prepPrompts() + self._conversation
        else:
            conversation = self._prepPrompts() + [newMessage]

        # Make the API request
        outputMessageText = self._makeRequest(conversation)
        timestamp = getTimeStamp()  # Get timestamp
        assistantType = "LLM"
        outputMessage = encodeMessageInternal(outputMessageText, timestamp, "assistant", assistantType)  # Package message

        # If history is enabled, store the assistant's response
        if self._history:
            self.insertMessageDict(outputMessage)
    ...
    def insertMessageDict(self, newMessage: dict):
        if not self._history:
            raise ValueError("History is disabled for this conversation instance.")
    ...
    def _makeRequest(self, tempConversation: list[dict]) -> str:
        # Make the API request
        ...
        logging.info(f"Response received: {output}")
        return output.strip()
